...
/*double TriangleArea(Point pt0, Point pt1, Point pt2) {
	...
	e = Length(pt0, pt2);
	s = (f + d + e) / 2;
	 area = sqrt(s*(s - f)*(s - d)*(s - e));
	return area; 
} */
void PolyArea(Polygon &thePoly, double *area) {
	double f, d, e, s;
	const unsigned numTriangles = thePoly.numSides - 2;
	 area[0] = 0;
	for (unsigned i = 1; i <= numTriangles; i++) {
		f = Length(thePoly.v[0], thePoly.v[i]);
		d = Length(thePoly.v[i], thePoly.v[i + 1]);
		e = Length(thePoly.v[0], thePoly.v[i + 1]);
		s = (f + d + e) / 2;
		area[i] = sqrt(s*(s - f)*(s - d)*(s - e));
		area[0] = area[0] + area[i];
	}
}
	/*const unsigned numTriangles = thePoly.numSides - 2;
